...
# (set via /etc/modprobe.d/8812eu.conf).
#
...
if ! lsmod | grep -q "^8812eu"; then
    fail "8812eu module not loaded. Load with: insmod 8812eu.ko rtw_cooperative_rx=1"
fi

COOP_PARAM=$(cat /sys/module/8812eu/parameters/rtw_cooperative_rx 2>/dev/null)
if [ "$COOP_PARAM" != "1" ]; then
    fail "Module loaded without rtw_cooperative_rx=1. Add to /etc/modprobe.d/8812eu.conf and reload"
fi
...
if [ "$IFACE_COUNT" -lt 2 ]; then
    fail "Need 2 RTL8812EU interfaces, found $IFACE_COUNT:$IFACES"
fi
